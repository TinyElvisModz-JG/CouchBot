...
{
    [Group("youtube")]
    public class YouTube : SecurityModule
    {
        IYouTubeManager _youTubeManager;
        IGuildManager _guildManager;

        public YouTube(DiscordShardedClient client, IYouTubeManager youtubeManager, IGuildManager guildManager) : base(client)
        {
        ...
        public async Task Add(string channelId)
        {
            ...
            if(guild.GuildChannels == null)
            {
                guild.GuildChannels = new List<Domain.Dtos.Discord.GuildChannel>();
            }
            var youtubeChannels = guild.GuildChannels.Where(gc => gc.Channel.PlatformId == (int)Domain.Enumerations.Platform.YouTube);
